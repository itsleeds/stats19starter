...
casualties_sf <- stats19::format_sf(casualties_joined, lonlat = TRUE)
casualties_sf |>
  sf::st_geometry() |>
  plot()
```
...
cardiff_boundary <- sf::st_union(cardiff_zones)
mapview::mapview(cardiff_boundary)
sf::st_crs(cardiff_boundary)
sf::st_crs(casualties_sf)

casualties_cardiff <- casualties_sf[cardiff_boundary, ]
...
sf::write_sf(casualties_cardiff, "casualties_cardiff.gpkg")
```


```{r}
# find the osmextract and osmactive packages and write code to get road data for Cardiff:
roads_cardiff <- osmactive::get_travel_network(
    place = cardiff_boundary,
    boundary = cardiff_boundary,
    boundary_type = "clipsrc"
)
sf::write_sf(roads_cardiff, "roads_cardiff.gpkg")

plot(sf::st_geometry(roads_cardiff))
```


```{r}
system("gh release create v1")
system("gh release upload v1 casualties_cardiff.gpkg roads_cardiff.gpkg")
```


```{r}
#' https://github.com/itsleeds/stats19starter/releases 
roads_cardiff <- sf::st_read(
    "https://github.com/itsleeds/stats19starter/releases/download/v1/roads_cardiff.gpkg"
)
```